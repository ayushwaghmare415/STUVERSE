...
export function VendorNavbar({ onMenuClick, isMobileMenuOpen, className }: { onMenuClick: () => void, isMobileMenuOpen: boolean, className?: string }) {
      ...
      <div className="flex items-center gap-4 ml-auto">
          ...
          {isNotificationsOpen && (
            <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="max-h-96 overflow-y-auto">
                <div className="px-4 py-3 hover:bg-slate-50 cursor-pointer flex gap-3 transition-colors bg-indigo-50/50">
                  <div className="h-8 w-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0">
                    <Bell className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-sm text-slate-900 font-medium">Offer Approved!</p>
                    <p className="text-xs text-slate-500 mt-0.5">"50% OFF Frappuccinos" is now live.</p>
                ...
                <div className="px-4 py-3 hover:bg-slate-50 cursor-pointer flex gap-3 transition-colors">
                  ...
                  <div>
                    <p className="text-sm text-slate-900 font-medium">New Redemption</p>
                    <p className="text-xs text-slate-500 mt-0.5">Ayush redeemed "50% OFF Frappuccinos".</p>
        ...
        <button className="flex items-center gap-2 p-1 pr-3 rounded-full hover:bg-slate-50 border border-transparent hover:border-slate-200 transition-all">
          <img 
            ...
            alt="Vendor Profile" 
            className="h-8 w-8 rounded-full object-cover ring-2 ring-white border border-slate-100"
            referrerPolicy="no-referrer"
          />
          <span className="text-sm font-medium text-slate-700 hidden sm:block">Your Store</span>
        </button>
      </div>
    </header>
  );
